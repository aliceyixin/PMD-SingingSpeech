...
class phlvlPMDMetricStats(BaseMetricStats):
        ...
        if len(self.saved_seqs) == 0:
            ...
            for key in self.saved_seqs:
                if key != 'utt_ids':
                    ...
                    # print(len(self.saved_seqs['utt_ids']), self.saved_seqs['utt_ids'])

                
        # print('epoch ids len', len(self.ids))
        # print('self.saved_seqs id len: ', len(self.saved_seqs['utt_ids']), len(self.scores_list))
        # print(self.scores_list['F1class_list'])
        # print(len(self.saved_seqs['utt_ids']), len(self.saved_seqs['gt_pmd_lbl_seqs']), len(self.saved_seqs['pred_pmd_lbl_seqs']))
    ...
    def summarize(self, field=None):
        ...
        # method 1: Micro average: calculate F1 for each instance
        ...
        # method 2: calculate metrics for each sample
        # ACC_instance = mean_scores['ACC_instance']
        # PRE_instance = mean_scores['PRE_instance']
        # REC_instance = mean_scores['REC_instance']
        # mean_scores['F1_instance'] = (2 * PRE_instance * REC_instance) / (PRE_instance + REC_instance + eps)
...
def seq_pmd_scoring(sr, n_class, prediction, target, note_on_seq, note_off_seq, seg_on_seq, seg_off_seq):  # frame level scoring
    ...
    """
#  (pred_pmd_lbl_seqs[i], gt_pmd_lbl_seqs[i],
#  gt_seg_on_seqs[i], gt_seg_off_seqs[i], 
#  pred_seg_on_seqs[i], pred_seg_off_seqs[i])

    # print('prediction and target: ', prediction, target)
    # print('on prediction and target: ', len(seg_on_seq), len(note_on_seq))
    # print('off prediction and target: ', len(seg_off_seq), len(note_off_seq))
    ...
    for cl in range(1, n_class):  # ignore 'rest'
        ...
        # print('TP ', TP)
        # print('FP, pred ', FP, np.sum(prediction_onehot[:,cl]))
        # print('FN, targ ', FN, np.sum(target_onehot[:,cl]))   
        # exit() 
        eps = 1e-6
...
def batch_seq_pmd_scoring(
    ...
    # print(pred_pmd_lbl_seqs, gt_pmd_lbl_seqs)
    # check input
    ...
    # print('pmd_scores', pmd_scores)  # batch pmd score
...
def batch_write_results(
    ...
    # print('B', B)
    # print('len score list', len(scores_list))

    # print('gt_pmd_lbl_seqs', len(gt_pmd_lbl_seqs))
    # print('pred_pmd_lbl_seqs', len(pred_pmd_lbl_seqs))

    # assert len(gt_pmd_lbl_seqs) == B
    # assert len(pred_pmd_lbl_seqs) == B
