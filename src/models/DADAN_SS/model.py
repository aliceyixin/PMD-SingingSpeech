...
from modules.normalization import l2_norm, batch_norm, stand_norm
...
class SBModel(PMDModel):
    ...
    def compute_forward(self, batch, stage):
        ...
        feats = self.modules['normalizer'](feats, feat_lens)

        batch_size = feats.shape[0] 

        if self.hparams.source == 'PMRead' and self.hparams.target == 'PMSing':
            source_id = 0
            target_id = 1
        elif self.hparams.source == 'PMSing' and self.hparams.target == 'PMRead':
            source_id = 1
            target_id = 0
        else:
            raise ValueError('Invalid source/target dataset.')

        # separate source input feature
        indices_source = torch.tensor(np.arange(source_id, batch_size, 2)).to(self.device)
        feats_source = torch.index_select(feats, 0, indices_source)
        # separate target input feature
        indices_target = torch.tensor(np.arange(target_id, batch_size, 2)).to(self.device)
        feats_target = torch.index_select(feats, 0, indices_target)
        
        # ====== input factorization using NMF ======
        # Encoder input size:(BATCH, F, T)
        feats = torch.cat((feats_source, feats_target), dim=0)
        ...
        if self.hparams.embedding_norm == 'l2':
            embedding_z = l2_norm(embedding_z)
            embedding_zd = l2_norm(embedding_zd)
        elif self.hparams.embedding_norm == 'batch':
            embedding_z = batch_norm(embedding_z)
            embedding_zd = batch_norm(embedding_zd)
        elif self.hparams.embedding_norm == 'stand':
            embedding_z = stand_norm(embedding_z)
            embedding_zd = stand_norm(embedding_zd)
        elif self.hparams.embedding_norm == 'None':
            pass
        ...
        # embedding 需要 normalizaation
        mine_loss = self.modules['MINE'](embedding_z, embedding_zd)   
        if mine_loss < 0:
            mine_loss = torch.tensor(0.).to(self.device)

        if self.hparams.compute_macs:
            Encoder_macs, Encoder_params = profile(
                self.modules['Encoder'], inputs=(feats,), verbose=False
            )
            DEncoder_macs, DEncoder_params = profile(
                self.modules['DEncoder'], inputs=(feats,), verbose=False
            )
            Classifier_macs, Classifier_params = profile(
                self.modules['Classifier'], inputs=(embedding_z,), verbose=False
            )
            DClassifier_macs, DClassifier_params = profile(
                self.modules['DClassifier'], inputs=(embedding_zd,), verbose=False
            )
            MINE_macs, MINE_params = profile(
                self.modules['MINE'], inputs=(embedding_z, embedding_zd), verbose=False
            )
            self.macs.append(np.sum([Encoder_macs, DEncoder_macs, Classifier_macs, DClassifier_macs, MINE_macs]))
            self.params.append(np.sum([Encoder_params, DEncoder_params, Classifier_params, DClassifier_params, MINE_params]))

        predictions = {
            'y': y,  # pred PMD label
            'embedding_z': embedding_z,  # Encoder 
    ...
    def compute_objectives(self, predictions, batch, stage):
        ...
        embedding_z_source, embedding_z_target = embedding_z.chunk(2, dim=0)
        ...
        source_loss, domain_pred_s = DomainADVLoss(self, embedding_z_source, True, lamb=lamb)
        target_loss, domain_pred_t = DomainADVLoss(self, embedding_z_target, False, lamb=lamb)
        domain_pred_s = domain_pred_s.reshape((domain_pred_s.size(0), domain_pred_s.size(1)))
        domain_pred_t = domain_pred_t.reshape((domain_pred_t.size(0), domain_pred_t.size(1)))
        ...
        # source singer gt
        indices_source = predictions['indices_source']
        phn_mode_label_seqs_source = torch.index_select(phn_mode_label_seqs, 0, indices_source)
        phlvl_phn_mode_label_len_source = torch.index_select(phlvl_phn_mode_label_len, 0, indices_source)
        flvl_phn_mode_label_seqs_source = torch.index_select(flvl_phn_mode_label_seqs, 0, indices_source)
        flvl_phn_mode_label_len_source = torch.index_select(flvl_phn_mode_label_len, 0, indices_source)
        
        flvl_phn_mode_pred_source = y_s.permute(0, 2, 1)
        flvl_phn_mode_label_source = flvl_phn_mode_label_seqs_source.data

        # enforce the pred and label have the same num of frames
        if flvl_phn_mode_pred_source.shape[-1] != flvl_phn_mode_label_source.shape[-1]:
            flvl_phn_mode_pred_source = flvl_phn_mode_pred_source[...,:flvl_phn_mode_label_source.shape[-1]]

        criterion_ce = nn.CrossEntropyLoss()  # Frame wise binary cross entropy loss
        ce_loss = criterion_ce(flvl_phn_mode_pred_source, flvl_phn_mode_label_source)  
        pmd_loss = ce_loss
        ...
        for domain in ['src', 'tgt']:
            if domain == 'src':
                out = y_s
                indices = predictions['indices_source']
            else:
                out = y_t
                indices = predictions['indices_target']
